...
from movie_analytics.metrics import genre_decade_summary
...
def database_url() -> URL:
    """Build an escaped SQLAlchemy URL from environment variables."""
    return URL.create(
        drivername="postgresql+psycopg2",
        username=os.getenv("POSTGRES_USER", "itc6050"),
...
def load_data() -> pd.DataFrame:
    """Read movie–genre rows from the transformed PostgreSQL layer."""
    ...
    return pd.read_sql(movies_sql, engine)


def main() -> None:
    """Render the complete interactive dashboard."""
    ...
    st.info(
        "This is a revenue-ranked sample, not a representative sample of the film "
        "industry. ROI is (reported revenue − production budget) / production budget; "
        "it excludes marketing, distribution and revenue sharing."
    )

    try:
        movie_rows = load_data()
    ...
    with st.sidebar:
        ...
        selected_genres = st.multiselect(
            "Genre", genres, default=genres[: min(5, len(genres))]
        )
        selected_decades = st.multiselect("Decade", decades, default=decades)
        max_vote_count = int(movie_rows["vote_count"].max())
        min_vote_count = st.slider(
            "Minimum vote count",
            min_value=0,
            max_value=max_vote_count,
    ...
    kpi3.metric(
        ...
        highest_grossing["title"],
        f"${highest_grossing['revenue']:,.0f}",
    )
